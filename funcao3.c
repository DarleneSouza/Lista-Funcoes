...
int ordenada(tlista l){
	if(!isEmpty(&l))
		while(l.head->prox != NULL){
			if(l.head->dado > l.head->prox->dado){
				return 0;
			}
			l.head=l.head->prox;
		}
...
void removePares(tlista *lista){
	tlista l = *lista;
	if(!isEmpty(&l))
		while(l.head!=NULL){
			if(l.head->dado % 2 == 0){
				removeLista(lista,l.head->dado);
			}
			l.head=l.head->prox;
		}
}
